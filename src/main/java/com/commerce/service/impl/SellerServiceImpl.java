...
import com.commerce.service.LoginLogoutService;
...
import com.commerce.exception.SellerException;
...
import com.commerce.repository.SellerDao;
import com.commerce.repository.SessionDao;
	...
	public Seller deleteSellerById(Integer sellerId, String token) {
		...
		//	UserSession user = sessionDao.findByToken(token).get();
			UserSession user = sessionDao.findByToken(token)
					.orElseThrow(()-> new SellerException("Invalid or expired session token"));
...
//			if(user.getUserId() == existingseller.getSellerId()) {
			if (user.getUserId().equals(existingseller.getSellerId())){
				...
				productDao.deleteBySellerId(existingseller.getSellerId());
